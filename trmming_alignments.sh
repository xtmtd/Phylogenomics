    ...
    then
      ...
      until [ -x $DIR_TRIMAL/trimal ]
        do
          ...
          DIR_TRIMAL=$(echo $DIR_TRIMAL_TEMP | sed "s/'//g")
        done
      echo "trimal ...... OK"
  fi
    ...
    then
      ...
      until [ -s $DIR_BMGE/BMGE.jar ]
        do
          ...
          DIR_BMGE=$(echo $DIR_BMGE_TEMP | sed "s/'//g")
        done
      echo "BMGE ...... OK"
  fi
...
if [ "$TRIM_METHOD" == "3" ]; then
    ...
    then
      ...
      until [ -x $DIR_CLIPKIT/clipkit ]
        do
          ...
          DIR_CLIPKIT=$(echo $DIR_CLIPKIT_TEMP | sed "s/'//g")
        done
      echo "ClipKIT ...... OK"
  fi
  ...
  if [ "$CLIPKIT_METHOD" == "1" ]; then
    cat loci.list | parallel -j $THREADS $DIR_CLIPKIT/clipkit {} -o {}.fas -m smart-gap
  elif [ "$CLIPKIT_METHOD" == "2" ]; then
    cat loci.list | parallel -j $THREADS $DIR_CLIPKIT/clipkit {} -o {}.fas -m kpi-smart-gap
  elif [ "$CLIPKIT_METHOD" == "3" ]; then
    cat loci.list | parallel -j $THREADS $DIR_CLIPKIT/clipkit {} -o {}.fas -m kpic-smart-gap
  fi
  cat loci.list | parallel -j $THREADS rm {}
...
if [ "$TRIM_METHOD" == "1" ] && [ "$ALIGN_TYPE" == "1" ]; then
  ...
  INPUT_AA=$(echo $INPUT_TEMP1 | sed "s/'//g")
  INPUT_NUC=$(echo $INPUT_TEMP2 | sed "s/'//g")
  ...
  if [ "$TRIMAL_METHOD" == "1" ]; then
    cat loci.list | parallel -j $THREADS $DIR_TRIMAL/trimal -in {}.faa -out {}.fas -automated1
    cat loci.list | parallel -j $THREADS $DIR_TRIMAL/trimal -in {}.faa -out {}.nuc.fas -automated1 -backtrans {}.fna
  elif [ "$TRIMAL_METHOD" == "2" ]; then
    cat loci.list | parallel -j $THREADS $DIR_TRIMAL/trimal -in {}.faa -out {}.fas -gappyout
    cat loci.list | parallel -j $THREADS $DIR_TRIMAL/trimal -in {}.faa -out {}.nuc.fas -gappyout -backtrans {}.fna
  fi
  cat loci.list | parallel -j $THREADS rm {}.faa {}.fna
...
if [ "$TRIM_METHOD" == "1" ] && [ "$ALIGN_TYPE" == "2" ]; then
  ...
  INPUT_NUC=$(echo $INPUT_TEMP | sed "s/'//g")
  ...
  cd $DIR_OUTPUT
  if [ "$TRIMAL_METHOD" == "1" ]; then
    cat loci.list | parallel -j $THREADS $DIR_TRIMAL/trimal -in {}.fna -out {}.fas -automated1
  elif [ "$TRIMAL_METHOD" == "2" ]; then
    cat loci.list | parallel -j $THREADS $DIR_TRIMAL/trimal -in {}.fna -out {}.fas -gappyout
  fi
  cat loci.list | parallel -j $THREADS rm {}.fna
  mv *fas trimal_fna
  .
...
if [ "$TRIM_METHOD" == "2" ] && [ "$ALIGN_TYPE" == "1" ]; then
  ...
  if [ "$BMGE_METHOD" == "1" ]; then
    cat loci.list | parallel -j $THREADS java -jar $DIR_BMGE/BMGE.jar -i {} -t AA -of {}.fas
  elif [ "$BMGE_METHOD" == "2" ]; then
    cat loci.list | parallel -j $THREADS java -jar $DIR_BMGE/BMGE.jar -i {} -t AA -m BLOSUM90 -h 0.4 -of {}.fas
  fi
  cat loci.list | parallel -j $THREADS rm {}
  mv *fas bmge_faa
fi
...
if [ "$TRIM_METHOD" == "2" ] && [ "$ALIGN_TYPE" == "2" ]; then
  ...
  cd $DIR_OUTPUT
  if [ "$BMGE_METHOD" == "1" ]; then
    cat loci.list | parallel -j $THREADS java -jar $DIR_BMGE/BMGE.jar -i {} -t DNA -of {}.fas
  elif [ "$BMGE_METHOD" == "2" ]; then
    cat loci.list | parallel -j $THREADS java -jar $DIR_BMGE/BMGE.jar -i {} -t DNA -m DNAPAM50 -h 0.4 -of {}.fas
  fi
  cat loci.list | parallel -j $THREADS rm {}
  mv *fas bmge_fna
fi
