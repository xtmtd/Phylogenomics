...
#Type 'sh loci_filtering_tree-based.sh'
...
if [ "$FILTER_METHOD" == "1" ]; then
  ...
  INPUT_ROOT=$(echo $INPUT_ROOT_TEMP | sed "s/'//g")
  ...
  #DVMC_fun() {
  #  dvmc=$(phykit dvmc -t $1 -r $INPUT_ROOT_TEMP)
  #  echo -e $1"\t"$dvmc > $1.dvmc
  #}
  #export -f DVMC_fun
  #cat ../loci.list | $DIR_PARALLEL/parallel -I% -j $THREADS --max-args 1 DVMC_fun %
  for loci in $(cat ../loci.list); do dvmc=$(phykit dvmc -t $loci -r $INPUT_ROOT); echo -e $loci"\t"$dvmc > $loci.dvmc; done
