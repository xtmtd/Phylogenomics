...
#copy the busco results to 0-raw_busco/
cd 0-raw_busco
for loci in $(cat ../species.list)
do
	mkdir run_$loci/
    cp -r ../tmp/$loci/run_*/busco_sequences/single_copy_busco_sequences/ run_$loci/
    cp -r ../tmp/$loci/run_*/busco_sequences/fragmented_busco_sequences/ run_$loci/
    cp ../tmp/$loci/run_*/*.txt run_$loci/
done
cd ..

for loci in $(cat species.list)
do
mv 0-raw_busco/run_$loci/fragmented_busco_sequences/* 0-raw_busco/run_$loci/single_copy_busco_sequences/
rm -rf 0-raw_busco/run_$loci/fragmented_busco_sequences/
done
