...
if [ "$TRIM_METHOD" == "3" ]; then
  ...
  #ClipKIT trimming
  read -p "Please input the folder names (with its path) containing aminoacid/nucleotide alignments, for example '3-faa_align':      " DIR_INPUT_TEMP
...
if [ $(which phykit) ]
   ...
   for LOCI in $(cat loci.list); do $DIR_PHYKIT/phykit thread_dna -p clipkit-faa/$LOCI -n temp_nuc/$LOCI -c $LOCI.log > clipkit-fna/$LOCI; done
   rm -rf temp_nuc/ *.log
  elif [ "$ALIGN_TYPE" == "2" ]; then
