...
#parallel, R, python, cscvtk, TreeShrink, ASTRAL, PhyKIT and Seqkit may be required
...
read -p "Please input the option for tree-based strategy for loci filtering: 1. average bootstrap support (ABS); 2. degree of violation of the molecular clock (DVMC); 3. treeness; 4. signal-to-noise ratio (treeness over RCV); 5. spurious homologs identification; 6. treeshrink; 7. inconsistent genes     " FILTER_METHOD
  until [ $FILTER_METHOD -gt 0 -a $FILTER_METHOD -lt 8 ]
    do
      read -p "Please input the option for tree-based strategy for loci filtering: 1. average bootstrap support (ABS); 2. degree of violation of the molecular clock (DVMC); 3. treeness; 4. signal-to-noise ratio (treeness over RCV); 5. spurious homologs identification; 6. treeshrink; 7. inconsistent genes     " FILTER_METHOD
    done
...
if [ "$FILTER_METHOD" == "1" ]; then
  ...
  rm ../spurious_homolog.partition ../spurious_homolog.fa ../list ../temp
...
for loci in $(cat loci.list);
  ...
  echo -e '\n'
  echo "Treeshrink with $THRESHOLD_TOR (threshold) has preserved $(cat $DIR_OUTPUT/TOR.partition | tail -n -1 | cut -d "=" -f2 | cut -d "-" -f2) sites and $(cat $DIR_OUTPUT/loci.TOR | wc -l) loci (the average site of each locus is $AVE)." | tee -a $DIR_OUTPUT/summary.statistics
