...
mkdir -p ${DIR_OUTPUT}/gene_trees  ${DIR_OUTPUT}/gene_trees_logs
ls ${DIR_INPUT} > ../temp.list

ln -s ${EXE_IQTREE} iqtree2
cat ../temp.list | ${DIR_PARALLEL}/parallel -j ${JOBS} ./iqtree2 -s {} ${MODEL} -B 1000 -T ${THREADS}
mv *treefile ${DIR_OUTPUT}/gene_trees/
mv *log *iqtree ${DIR_OUTPUT}/gene_trees_logs/
cd ..
