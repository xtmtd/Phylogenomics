...
#All the BUSCO results (run_* folders generated from BUSCO v3; $SPECIES folders generated from BUSCO v5.4) are deposited in the same folder, e.g. BUSCOs/
...
#Generate a file of loci list
  ...
  for SPECIES in ${SPECIES_NAME}
    do
      ...
      ls | sed 's/.fna\|.faa//g' >> ../../../temp.list
      cd ../../..
    done
  sort -u temp.list > loci.list
  rm temp.list
    ...
    do
      if [ -f 0-raw_busco/run_${SPECIES}/single_copy_busco_sequences/${LOCI}.fna ]
          then
            ...
            echo "${LOCI} in ${SPECIES} does not exist" >> 1-raw_loci/log.txt
      fi
    done
  done
...
echo "Copying BUSCO files ......"
  ...
  ls ${DIR_INPUT}/ | sed 's/ //g' > species.list
...
#copy the busco results to 0-raw_busco/
  ...
  for SPECIES in $(cat ../species.list)
    do
      mkdir run_${SPECIES}/
      cp -r ${DIR_INPUT}/${SPECIES}/run_*/busco_sequences/single_copy_busco_sequences/ run_${SPECIES}/
      cp ${DIR_INPUT}/${SPECIES}/run_*/*.txt run_${SPECIES}/
    done
  cd ..
...
echo 
echo "Filter loci having too few taxa (less than three) ......"

mkdir -p 2-loci_filter/fna 2-loci_filter/faa
TOTAL_TAXA=$(cat species.list | wc -l)
for LOCI in ${LOCI_NAME}
do
    echo -e ${LOCI}'\t'$(grep -o "^${LOCI}" 1-raw_loci/log.txt | wc -l) >> 2-loci_filter/sequence_number.log
